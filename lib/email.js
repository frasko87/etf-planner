...
const FROM = "ETF.PLAN <onboarding@resend.dev>"; // use until custom domain verified
...
// ── 4. Newsletter ────────────────────────────────────────────────────────────
export async function sendNewsletter({ email, name, subject, headline, body, ctaText, ctaUrl }) {
  const html = `<!DOCTYPE html><html><head>${BASE}</head><body>
    <div class="container">
      <div class="logo">ETF<span>.</span>PLAN</div>
