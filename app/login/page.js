// app/login/page.js
// Fix: emailRedirectTo → /dashboard (not homepage). Auto logged-in after email confirm.
"use client";
...
function LoginForm() {
    ...
    try {
      if (mode === "signup") {
        const { error: err } = await supabase.auth.signUp({
          email, password,
          options: {
            // FIXED: was window.location.origin (homepage) — now correctly points to /dashboard
  ...
  const inp = { width:"100%", padding:"12px 14px", background:"var(--bg3)", border:"1.5px solid var(--border)", borderRadius:10, color:"var(--text)", fontFamily:"DM Sans", fontSize:14 };
    ...
    <div style={{ minHeight:"100vh", background:"var(--bg)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24 }}>
      ...
      <div style={{ width:"100%", maxWidth:420, background:"white", border:"1px solid var(--border)", borderRadius:20, padding:36, boxShadow:"var(--shadow2)" }}>
        ...
        <div style={{ display:"flex", background:"var(--bg3)", borderRadius:12, padding:4, marginBottom:28 }}>
          {["login","signup"].map(m => (
            <button key={m} onClick={() => { setMode(m); setError(""); setSuccess(""); }} style={{ flex:1, padding:"10px 0", borderRadius:9, border:"none", cursor:"pointer", transition:"all 0.15s", background:mode===m?"white":"transparent", color:mode===m?"var(--text)":"var(--muted)", fontFamily:"DM Sans", fontWeight:mode===m?600:400, fontSize:14, boxShadow:mode===m?"var(--shadow)":"none" }}>
        ...
        {mode === "login" && (
          <div style={{ textAlign:"right", marginBottom:16 }}>
            <button onClick={handleReset} disabled={loading} style={{ background:"none", border:"none", color:"var(--green)", cursor:"pointer", fontFamily:"DM Sans", fontSize:12, fontWeight:500 }}>Forgot password?</button>
          </div>
        )}
        ...
        <button onClick={handleSubmit} disabled={loading} style={{ width:"100%", padding:"13px 0", borderRadius:10, border:"none", background:loading?"var(--border)":"var(--green)", color:loading?"var(--muted)":"white", fontFamily:"DM Sans", fontWeight:600, fontSize:15, cursor:loading?"not-allowed":"pointer", boxShadow:loading?"none":"0 4px 16px rgba(0,185,107,0.3)", transition:"all 0.15s" }}>
          {loading ? "Please wait…" : mode === "login" ? "Log in →" : "Create account →"}
        </button>
        <p style={{ fontSize:12, color:"var(--muted)", textAlign:"center", marginTop:20 }}>
          {mode === "login" ? "No account? " : "Already have one? "}
          <button onClick={() => { setMode(mode==="login"?"signup":"login"); setError(""); setSuccess(""); }} style={{ background:"none", border:"none", color:"var(--green)", cursor:"pointer", fontFamily:"DM Sans", fontSize:12, fontWeight:500 }}>
            {mode === "login" ? "Sign up free" : "Log in"}
          </button>
        </p>
      </div>
      <p style={{ fontSize:11, color:"var(--muted2)", marginTop:24, textAlign:"center" }}>Not financial advice. Past performance ≠ future results.</p>
    </div>
  );
}
