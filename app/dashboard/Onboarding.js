...
// Added: first name capture on Step 0 (welcome screen)
// - Optional text input "What should we call you?"
// - Saves to Supabase auth.users raw_user_meta_data via supabase.auth.updateUser()
// - Also saves to user_plans.first_name for easy querying
// - Used in: done screen greeting, dashboard "Hi, X", welcome email subject
// - Skip allowed: if left blank, falls back to email prefix as before
"use client";
...
export default function Onboarding({ user, onComplete }) {
  ...
  const [firstName, setFirstName] = useState(""); // ← NEW: name capture
  ...
  // Display name logic — uses entered name, falls back to email prefix
  const displayName = firstName.trim() || user?.email?.split("@")[0] || "investor";
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      // ── Save name to Supabase auth user metadata (if provided) ─────────────
      if (firstName.trim()) {
        await supabase.auth.updateUser({
          data: { first_name: firstName.trim(), full_name: firstName.trim() }
        });
      }
      ...
      const { error: planErr } = await supabase.from("user_plans").upsert({
        ...
        // Store name in user_plans too for easy email personalisation
        ...(firstName.trim() ? { first_name: firstName.trim() } : {}),
      });
      if (planErr) throw planErr;

      // ── Save monthly action ────────────────────────────────────────────────
      const { error: actionErr } = await supabase.from("user_monthly_actions").upsert({
        user_id: user.id,
        month_key: monthKey,
        profile,
        amount,
        tickers: pc.etfs,
        allocations: Object.fromEntries(pc.etfs.map((t,i) => [t, pc.allocs[i] || Math.floor(100/pc.etfs.length)])),
        entry_prices: {},
        amounts_invested: Object.fromEntries(pc.etfs.map((t,i) => [t, Math.round(amount * (pc.allocs[i] || Math.floor(100/pc.etfs.length)) / 100)])),
      });
      if (actionErr) throw actionErr;

      // ── Meta Pixel ─────────────────────────────────────────────────────────
      if (typeof window !== "undefined" && window.fbq) {
        window.fbq("track","CompleteRegistration",{content_name:`${profile}_${amount}`,currency:"USD",value:amount});
      }
      ...
      if (utms.ref) {
        await fetch("/api/referral",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code:utms.ref,newUserId:user.id,newUserEmail:user.email})}).catch(()=>{});
      }
      ...
      track("onboarding_completed",{profile,amount,has_name:!!firstName.trim()});
      setDone(true);
    } catch(e) {
    ...
    return (
      <div style={stepStyle}>
        <div style={{...card, maxWidth:520, textAlign:"center"}}>
          ...
          <div className="pixel" style={{fontSize:11, color:"var(--text)", marginBottom:16}}>
            ETF<span style={{color:"var(--green)"}}>.</span>PLAN
          </div>
          {/* Uses the captured name or email prefix */}
          <h2 style={{fontFamily:"DM Sans",fontWeight:700,fontSize:"clamp(22px,4vw,30px)",color:"var(--text)",marginBottom:8,letterSpacing:"-0.5px"}}>
            Step 1 done{firstName.trim() ? `, ${firstName.trim()}` : ""}. Now buy your ETFs.
          </h2>
          ...
          <button onClick={() => onComplete({ profile, amount, firstName: firstName.trim() || null })} style={{width:"100%",padding:"16px 0",borderRadius:12,border:"none",background:"var(--green)",color:"white",fontFamily:"DM Sans",fontWeight:700,fontSize:16,cursor:"pointer",boxShadow:"0 4px 20px rgba(0,185,107,0.3)"}}>
            Go to my dashboard →
          </button>
    ...
    return (
      <div style={stepStyle}>
        <div style={card}>
          ...
          {/* Hero card */}
          <div style={{background:"var(--text)",borderRadius:14,padding:"24px 20px",marginBottom:24,textAlign:"center"}}>
            <div style={{fontSize:48, marginBottom:12}}>👋</div>
            <h1 style={{fontFamily:"DM Sans",fontWeight:700,fontSize:"clamp(22px,4vw,28px)",color:"white",marginBottom:10,letterSpacing:"-0.5px"}}>
              Welcome to ETF.PLAN
            </h1>
            <p style={{fontFamily:"DM Sans",fontSize:15,color:"rgba(255,255,255,0.55)",lineHeight:1.7,margin:0}}>
              Let's build your personalised saving plan. Takes 2 minutes — your choices are saved so every month you know exactly what to buy.
            </p>
          </div>

          {/* ── NAME INPUT — the new addition ───────────────────────────── */}
          <div style={{marginBottom:20}}>
            <label style={{fontFamily:"DM Sans",fontWeight:600,fontSize:14,color:"var(--text)",display:"block",marginBottom:8}}>
            ...
            <input
              ...
              placeholder={`e.g. ${emailPrefix ? emailPrefix.charAt(0).toUpperCase() + emailPrefix.slice(1,12) : "Alex"}`}
              maxLength={30}
              autoFocus
              style={{
                width: "100%",
              ...
              onFocus={e => { if(!firstName.trim()) e.target.style.borderColor = "rgba(0,185,107,0.4)"; }}
              onBlur={e => { if(!firstName.trim()) e.target.style.borderColor = "var(--border)"; }}
            />
            {/* Skip hint */}
            <p style={{fontFamily:"DM Sans",fontSize:12,color:"var(--muted2)",marginTop:6,marginBottom:0}}>
              Optional — we'll use your email if you skip
            </p>
          </div>
          {/* ─────────────────────────────────────────────────────────────── */}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:28}}>
            {[
              ...
              {icon:"📊", text:"See exactly which ETFs to buy each month"},
              {icon:"📈", text:"Track your real results month by month"},
            ].map(s => (
              <div key={s.text} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:"var(--bg3)",borderRadius:10}}>
                <span style={{fontSize:16}}>{s.icon}</span>
                <span style={{fontFamily:"DM Sans",fontSize:13,color:"var(--text)"}}>{s.text}</span>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={() => setStep(1)}
            style={{width:"100%",padding:"16px 0",borderRadius:12,border:"none",background:"var(--green)",color:"white",fontFamily:"DM Sans",fontWeight:700,fontSize:17,cursor:"pointer",boxShadow:"0 4px 20px rgba(0,185,107,0.3)"}}
          >
            {firstName.trim() ? `Let's go, ${firstName.trim()} →` : "Let's set up my plan →"}
          </button>
          ...
          {!firstName.trim() && (
            <button
              onClick={() => setStep(1)}
              style={{width:"100%",marginTop:10,padding:"8px 0",background:"none",border:"none",color:"var(--muted2)",fontFamily:"DM Sans",fontSize:12,cursor:"pointer",textDecoration:"underline",textDecorationColor:"transparent"}}
  ...
  if (step === 1) return (
    <div style={stepStyle}>
      <div style={card}>
        ...
        <button onClick={() => { setStep(2); track("onboarding_step_1",{amount}); }} style={{width:"100%",padding:"16px 0",borderRadius:12,border:"none",background:"var(--text)",color:"white",fontFamily:"DM Sans",fontWeight:700,fontSize:16,cursor:"pointer"}}>
          Continue →
        </button>
      </div>
    </div>
    ...
    <div style={stepStyle}>
      <div style={{...card, maxWidth:560}}>
        ...
        <h2 style={{fontFamily:"DM Sans",fontWeight:700,fontSize:"clamp(22px,4vw,32px)",color:"var(--text)",marginBottom:6,letterSpacing:"-0.5px"}}>
          {firstName.trim() ? `${firstName.trim()}, your plan is ready` : "Your plan is ready"}
        </h2>
        <p style={{fontFamily:"DM Sans",fontSize:15,color:"var(--muted)",marginBottom:24,lineHeight:1.7}}>
