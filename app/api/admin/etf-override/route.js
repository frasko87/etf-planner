...
export async function POST(req) {
  ...
  // Save override
  await supabase.from("etf_overrides").upsert({
    profile,
    tickers,
    allocations,
    override_note: note,
  ...
  // Update weekly_selections so dashboard shows it immediately
  await supabase.from("weekly_selections")
  ...
  await supabase.from("weekly_selections").insert({
    ...
    is_current: true,
    change_summary: `Admin override: ${note || "manual"}`,
    changed: true,
  });
