"use client";
// app/es/blog/[slug]/page.js
// Spanish blog post — fetches by slug_es, renders Spanish content
...
export default function BlogPostES() {
  ...
  useEffect(() => {
    ...
    // Query by slug_es for Spanish posts
    supabase
      .from("blog_posts")
      .select("*")
      .eq("slug_es", slug)
      ...
      .single()
      .then(({ data, error }) => {
        if (error || !data) { router.replace("/es/blog"); return; }
        setPost(data);
        const title = data.meta_title_es || data.title_es || data.title;
        document.title = `${title} | ETF.PLAN Blog`;
  ...
  if (loading) return (
    <>
      ...
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:BG }}>
        <div style={{ fontFamily:"DM Mono,monospace", fontSize:12, color:"#aaaabc", letterSpacing:1 }}>Cargando...</div>
      </div>
    </>
  );
  ...
  // Use Spanish content, fall back to English
  ...
  const shareUrl = typeof window !== "undefined" ? window.location.href : `https://etfplan.app/es/blog/${slug}`;
    ...
    <>
      ...
      <header style={{ background:BG, borderBottom:`1px solid ${BOR}`, padding:"0 clamp(16px,4vw,40px)", height:56, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:50 }}>
        ...
        <div style={{ display:"flex", gap:"clamp(10px,2vw,20px)", alignItems:"center" }}>
          ...
          {post.slug && <Link href={`/blog/${post.slug}`} style={{ fontSize:12, color:MU, padding:"5px 10px", border:`1px solid ${BOR}`, borderRadius:6, textDecoration:"none" }}>🇬🇧 EN</Link>}
      ...
      <section style={{ background:D, padding:"clamp(48px,7vw,72px) clamp(16px,4vw,40px)" }}>
        <div style={{ maxWidth:720, margin:"0 auto" }}>
          <div style={{ display:"flex", gap:10, marginBottom:20, flexWrap:"wrap", alignItems:"center" }}>
            <Link href="/es/blog" style={{ fontSize:12, fontFamily:"DM Mono,monospace", color:"rgba(255,255,255,0.3)", textDecoration:"none" }}>← Blog</Link>
            ...
            <span style={{ fontSize:11, fontFamily:"DM Mono,monospace", color:"rgba(255,255,255,0.3)" }}>{post.reading_time} min de lectura</span>
          </div>
          <h1 style={{ fontSize:"clamp(24px,5vw,42px)", fontWeight:800, color:"white", lineHeight:1.15, letterSpacing:"-0.8px", marginBottom:16 }}>
            {title}
          </h1>
          {excerpt && <p style={{ fontSize:"clamp(14px,2vw,17px)", color:"rgba(255,255,255,0.5)", lineHeight:1.75, marginBottom:16 }}>{excerpt}</p>}
          {date && <p style={{ fontSize:13, fontFamily:"DM Mono,monospace", color:"rgba(255,255,255,0.25)", margin:0 }}>{post.writer_name || "ETF.PLAN"} · {date}</p>}
        </div>
      </section>

      {/* ── BODY ─────────────────────────────────────────────────────── */}
      <article style={{ maxWidth:720, margin:"0 auto", padding:"clamp(32px,5vw,56px) clamp(16px,4vw,24px)" }}>
        <div className="post-body" dangerouslySetInnerHTML={{ __html: content || "<p>Contenido próximamente.</p>" }} />

        {/* Share */}
        <div style={{ marginTop:48, padding:24, background:"white", border:`1px solid ${BOR}`, borderRadius:14 }}>
          <p style={{ fontSize:14, fontWeight:700, color:D, marginBottom:14 }}>¿Te ha sido útil? Compártelo:</p>
        ...
        <div style={{ margin:"32px 0", background:D, borderRadius:14, padding:"28px 24px" }}>
          ...
          <h3 style={{ fontSize:"clamp(17px,3vw,21px)", fontWeight:700, color:"white", marginBottom:8, letterSpacing:"-0.3px" }}>
            Tu plan de ETFs personalizado en 2 minutos
          </h3>
          <p style={{ fontSize:14, color:"rgba(255,255,255,0.5)", lineHeight:1.75, marginBottom:20 }}>
            Dinos cuánto quieres invertir al mes y tu nivel de riesgo. Nosotros calculamos qué ETFs comprar y te avisamos el 1 de cada mes. Gratis siempre.
          </p>
        ...
        {related.length > 0 && (
          <div style={{ marginTop:40 }}>
            <h2 style={{ fontSize:18, fontWeight:700, marginBottom:20, color:D }}>Artículos relacionados</h2>
            <div style={{ display:"grid", gridTemplateColumns:isMob?"1fr":"repeat(2,1fr)", gap:16 }}>
              {related.map(r => (
                <Link key={r.id} href={`/es/blog/${r.slug_es || r.slug}`} style={{ textDecoration:"none" }}>
                  <div className="rel-card" style={{ background:"white", border:`1px solid ${BOR}`, borderRadius:12, padding:20 }}>
                    ...
                    <h3 style={{ fontSize:15, fontWeight:700, color:D, lineHeight:1.4, marginBottom:8 }}>{r.title_es || r.title}</h3>
                    <span style={{ fontSize:13, color:G, fontWeight:600 }}>Leer →</span>
                  </div>
                </Link>
              ))}
            </div>
      ...
      <footer style={{ borderTop:`1px solid ${BOR}`, padding:"24px clamp(16px,4vw,40px)", marginTop:40 }}>
        <div style={{ maxWidth:720, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:12 }}>
          ...
          <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
            ...
            <Link href="/blog"     style={{ fontSize:13, color:MU, textDecoration:"none" }}>🇬🇧 English</Link>
          </div>
